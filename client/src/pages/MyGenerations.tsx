      ...
      >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, index) => (
              <motion.div
                ...
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.15, type: "spring", stiffness: 320, damping: 70, mass: 1 }} className="rounded-2xl bg-white/6 border border-white/10 animate-pulse h-[260px]"
              />
            ))}
          </div>
        )}
        ...
        {!loading && thumbnails.length === 0 && (
          <div className="text-center py-24">
            <h3 className="font-semibold text-lg text-zinc-200">No thumbnails yet.</h3>
        ...
        {!loading && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
            {thumbnails.map((thumb: IThumbnail, index) => {
                ...
                >
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs text-zinc-400">
                      ...
                      <span className="px-2 py-0.5 rounded bg-white/8 capitalize">{thumb.color_scheme}</span>
                  ...
                  <div onClick={(e) => e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1">
